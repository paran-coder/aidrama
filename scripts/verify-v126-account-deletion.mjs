...
const deletionAction = read('lib/actions/account-deletion.ts');
...
const checks = [
  ['server action exists', deletionAction.includes('deleteMyAccountAction')],
  ['admin self-delete blocked', deletionAction.includes('context.isAdmin') && deletionAction.includes('관리자 계정은')],
  ['typed confirmation enforced server-side', deletionAction.includes('confirmText !== "탈퇴"')],
  ['auth user is hard-deleted through admin client', deletionAction.includes('admin.auth.admin.deleteUser(context.user.id, false)')],
  ['session sign-out attempted', deletionAction.includes('supabase.auth.signOut()')],
