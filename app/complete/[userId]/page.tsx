...
export default async function CompletePage({ params }: { params: Promise<{ userId: string }> }) {
  ...
  if (!hasCompletionBadge(badges, progress.day)) notFound();
  return (
    <AppShell displayName={profile.display_name} isAdmin={isAdmin}>
      <div className="mx-auto max-w-4xl text-center">
        ...
        <p className="copy-pretty mx-auto mt-5 max-w-xl leading-8 text-[var(--muted)]">처음부터 창작자였고, 이제 1000일의 기록으로 그것을 증명했습니다. 실패한 주도, 다시 시작한 주도 모두 이 여정의 일부입니다.</p>
        ...
        <div className="mx-auto mt-6 max-w-2xl"><MilestoneBadges badges={badges} currentDay={progress.day} /></div>
