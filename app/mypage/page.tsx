...
export default async function MyPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}) {
  ...
  <section className={`card rounded-[2rem] p-6 ${!isAdmin && challenge ? "mt-4" : "mt-8"}`}><h2 className="text-xl font-black">프로필</h2><form action={updateProfileAction} className="mt-5 space-y-4"><label className="block text-sm font-bold">{isAdmin ? "표시 이름" : "톡방 닉네임"}<input className="input-field mt-2" name="displayName" defaultValue={profile.display_name} maxLength={40} required/></label><label className="block text-sm font-bold">이메일<input className="input-field mt-2" value={user.email??""} disabled readOnly/></label><PendingSubmitButton pendingLabel="저장 중...">정보 저장</PendingSubmitButton></form></section><section className="card mt-4 rounded-[2rem] p-6"><h2 className="text-xl font-black">비밀번호 변경</h2><form action={updatePasswordAction} className="mt-5 space-y-4"><label className="block text-sm font-bold">새 비밀번호<input className="input-field mt-2" name="password" type="password" minLength={8} required/></label><PendingSubmitButton className="secondary-button" pendingLabel="변경 중...">비밀번호 변경</PendingSubmitButton></form></section><section className="card mt-4 rounded-[2rem] p-6"><h2 className="text-xl font-black">계정 삭제 안내</h2><p className="copy-pretty mt-2 text-sm font-bold leading-6 text-[var(--muted)]">계정 삭제가 필요한 경우 운영자에게 요청해 주세요. 운영자가 본인 확인 후 계정과 활동 기록을 삭제합니다.</p></section></div></AppShell>;
}
