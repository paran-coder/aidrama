  ...
  return (
    <AppShell displayName={profile.display_name} isAdmin={isAdmin}>
      <div className="mx-auto max-w-3xl">
        ...
        {results.length === 0 ? (
          <div className="card mt-8 rounded-[2rem] p-8 text-center"><p className="text-lg font-black">아직 판정 이력이 없습니다.</p><p className="mt-2 text-sm text-[var(--muted)]">첫 주간 챌린지가 끝나면 이곳에 기록이 쌓입니다.</p></div>
        ) : (
            ...
            {results.map((row) => {
              ...
              return (
                <li key={row.id} className="card rounded-[1.5rem] p-5">
                  <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-extrabold text-[var(--muted)]">{row.week_start} 시작 주</p><p className="mt-1 text-lg font-black">{row.status === "success" ? "업로드 성공" : "업로드 없음"}</p></div><div className="flex gap-2"><StatusBadge status={row.status} />{submission && <StatusBadge status={submission.verification_status} />}</div></div>
